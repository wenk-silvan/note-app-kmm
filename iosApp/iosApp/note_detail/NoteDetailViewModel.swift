...
extension NoteDetailScreen {
    class NoteDetailViewModel: ObservableObject {
        ...
        func loadNoteIfExists(id: Int64?) {
            if id != nil {
                self.noteId = id
                noteDataSource?.getNote(id: id!, completionHandler: { note, error in
        ...
        func saveNote(onSaved: @escaping () -> Void) {
            noteDataSource?.insertNote(
                note: Note(id: noteId == nil ? nil : KotlinLong(value: noteId!), title: self.noteTitle, content: self.noteContent, colorHex: self.noteColor, created: DateTimeUtil().now()),
                completionHandler: { error in onSaved() })
        }
